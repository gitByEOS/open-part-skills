...
skill_dir/SKILL.md 自行决定怎么跑,把产物路径写入 work_dir/run_record.json。

run_record 契约:必填 artifacts(框架能核实路径合法性 + 存在性);可选 steps
(每步 command/exit_code/stdout/stderr,agent 自报过程,框架不核实)、envelope
(skill 最终 envelope,自报)。框架只校验它能核实的事实,自报部分供 agent_report
人读参考。
...
RUN_RECORD_REQUIRED_FIELDS = ["artifacts"]
RUN_RECORD_OPTIONAL_FIELDS = ["steps", "envelope"]
RUN_RECORD_STEP_FIELDS = ["command", "exit_code", "stdout", "stderr"]
...
class AgentRun(Node):
    ...
    def accept(self, ctx) -> bool:
        ...
        brief = {
            ...
            "run_record_optional_fields": RUN_RECORD_OPTIONAL_FIELDS,
            "run_record_step_fields": RUN_RECORD_STEP_FIELDS,
            "artifacts_must_under": work_dir_resolved,
            "resume_cmd": f"python3 scripts/run.py --resume {work_dir}",
            "cwd_must_be": work_dir_resolved,
        }
